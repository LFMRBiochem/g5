    ...
    watch:{
    	...
    	selectedConcepto:function(val, oldVal){
        	if(val!=null){
        		...
        		this.newItem.id_concepto_financiero = val.value;
    ...
    methods:{
        ...
        editItem: function (item) {
            ...
            this.$http.get('nmn_cat_conceptos/edit/' + item.id_folio_concepto).then((response) => {
                ...
                this.fillItem.id_concepto = item.id_concepto;
                this.fillItem.descripcion = item.descripcion;

                this.fillItem.percepcion_deduccion = item.percepcion_deduccion;
                this.fillItem.operacion = item.operacion;

                this.fillItem.id_concepto_financiero = item.id_concepto_financiero;
                this.fillItem.considerar_recibo = item.considerar_recibo;
                this.fillItem.considerar_reportes = item.considerar_reportes; //----------
        ...
        updateItem: function (id_empleado) {
            ...
            this.$http.put('nmn_cat_conceptosC/' + id_empleado, input).then((response) => {
                this.changePage(this.pagination.current_page);
                this.newItem = {
                    'id_folio_concepto': '',
		            'cve_compania': '',
